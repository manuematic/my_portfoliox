...
class StockSensor(CoordinatorEntity[MyPortfolioCoordinator], SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return {
            ...
            # OnVista Technik / Dividende / Termine / Analysten
            ATTR_OV_SMA_20:            data.get(ATTR_OV_SMA_20),
            ATTR_OV_SMA_200:           data.get(ATTR_OV_SMA_200),
            ATTR_OV_RSL_30:            data.get(ATTR_OV_RSL_30),
            ATTR_OV_RSL_250:           data.get(ATTR_OV_RSL_250),
            ATTR_OV_MOMENTUM_30:       data.get(ATTR_OV_MOMENTUM_30),
            ATTR_OV_MOMENTUM_250:      data.get(ATTR_OV_MOMENTUM_250),
            ATTR_OV_DIVIDENDE:         data.get(ATTR_OV_DIVIDENDE),
            ATTR_OV_DIVIDENDE_RENDITE: data.get(ATTR_OV_DIVIDENDE_RENDITE),
            ATTR_OV_NAECHSTER_TERMIN:  data.get(ATTR_OV_NAECHSTER_TERMIN),
            ATTR_OV_SIGNAL:            data.get(ATTR_OV_SIGNAL),
            ATTR_OV_KZ_MITTEL:         data.get(ATTR_OV_KZ_MITTEL),
            ATTR_OV_KZ_HOCH:           data.get(ATTR_OV_KZ_HOCH),
            ATTR_OV_KZ_TIEF:           data.get(ATTR_OV_KZ_TIEF),
            ATTR_OV_ANALYSTEN_ANZAHL:  data.get(ATTR_OV_ANALYSTEN_ANZAHL),
            ATTR_OV_ANALYSTEN_KONSENS: data.get(ATTR_OV_ANALYSTEN_KONSENS),
            ATTR_OV_ANALYSTEN_BUY:     data.get(ATTR_OV_ANALYSTEN_BUY),
            ATTR_OV_ANALYSTEN_HOLD:    data.get(ATTR_OV_ANALYSTEN_HOLD),
            ATTR_OV_ANALYSTEN_SELL:    data.get(ATTR_OV_ANALYSTEN_SELL),
        }
