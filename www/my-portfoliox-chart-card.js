/**
 * my-portfoliox-chart-card  v0.1.0
 * Kursverlauf + Kennzahlen-Panel
 * Daten kommen aus HA-Sensor-Attributen (preis_history, sma_20/50/200)
 *
 * YAML:
...
class MyPortfolioChartCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    if (!_chartState.has(this._uid)) {
      _chartState.set(this._uid, {
        ...
        sma20: false, sma50: false, sma200: false, trend: false, kursziel: false,
      });
    }
  }
  ...
  // ── Preishistorie aus HA-Sensor-Attribut ────────────────────────────────

  _getPrices(symbol) {
    const raw = _getSensorAttr(this._hass, symbol, "preis_history");
    if (!Array.isArray(raw) || raw.length < 2) return null;
    return raw.map(p => ({
      date:  new Date(p.date + "T12:00:00"),
      close: parseFloat(p.kurs),
  ...
  _drawChart(prices) {
    ...
    const sma50v  = st.sma50  ? this._sma(prices, 50)  : [];
    const sma200v = st.sma200 ? this._sma(prices, 200) : [];
    const trendv  = st.trend  ? this._trendline(prices) : [];

    if (st.sma20)  allVals.push(...sma20v.filter(v => v !== null));
    if (st.sma50)  allVals.push(...sma50v.filter(v => v !== null));
    ...
    return `
      <svg viewBox="0 0 ${W} ${H}" class="chart-svg" preserveAspectRatio="xMidYMid meet">
        ...
        ${st.sma50  ? smaPath(sma50v,  "#60a5fa") : ""}
  ...
  _render() {
    ...
    // Preisdaten direkt aus HA-Attribut lesen
    const prices = curSym ? this._getPrices(curSym) : null;
    ...
    let chartContent;
    if (!prices) {
      chartContent = `<div class="no-data">Noch keine Daten – InfluxDB wird beim nächsten Update befüllt.</div>`;
    } else {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .sma20-btn.active{background:rgba(52,211,153,.2);border-color:#34d399;color:#6ee7b7}
        .sma50-btn.active{background:rgba(96,165,250,.2);border-color:#60a5fa;color:#93c5fd}
      ...
      <ha-card>
        ...
        <div class="indicators">
          ...
          <button class="ind-btn sma20-btn${st.sma20?" active":""}" onclick="this.getRootNode().host._toggle('sma20')">SMA 20</button>
          <button class="ind-btn sma50-btn${st.sma50?" active":""}" onclick="this.getRootNode().host._toggle('sma50')">SMA 50</button>
          <button class="ind-btn sma200-btn${st.sma200?" active":""}" onclick="this.getRootNode().host._toggle('sma200')">SMA 200</button>
          <button class="ind-btn trend-btn${st.trend?" active":""}" onclick="this.getRootNode().host._toggle('trend')">Trend</button>
  ...
  _selectPortfolio(portfolio) {
    ...
    if (stocks.length > 0) this._set("symbol", stocks[0].kuerzel);
    this._render();
  }
...
customElements.define("my-portfoliox-chart-card", MyPortfolioChartCard);
window.customCards = window.customCards || [];
window.customCards.push({
  type:        "my-portfoliox-chart-card",
  name:        "Portfolio Kursverlauf",
  description: "Kursverlauf aus InfluxDB mit SMA 20/50/200 + Kennzahlen-Panel",
});
